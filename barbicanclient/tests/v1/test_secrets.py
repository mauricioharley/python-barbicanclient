...
class WhenTestingSecrets(test_client.BaseEntityResource):
    ...
    def test_should_delete_from_manager(self, secret_ref=None, consumers=None):
        # Secret with no consumers
        api_get_return = {
        ...
        self.client.client.get = mock.MagicMock()
        self.client.client.get.return_value = api_get_return

        secret_ref = secret_ref or self.entity_href

        delete_response = self.responses.delete(self.entity_href,
                                                status_code=204)

        # Verify the correct URL was used to make the call.
        self.assertEqual(self.entity_href, delete_response._url)

        # Calls the delete method only if secret has no consumers.
        # Otherwise leaves the correct method to delete it accordingly.
        if len(api_get_return['consumers']) == 0:
            self.manager.delete(secret_ref=secret_ref)
        else:
            return api_get_return, secret_ref

    def _test_should_delete_from_manager_with_consumers(self,
                                                        secret_ref=None,
                                                        force=False):
        consumers = [{'service': 'service_test',
                      'resource_type': 'type_test',
                      'resource_id': 'id_test'}]
        api_get_return, secret_ref = self.test_should_delete_from_manager(
            consumers=consumers)

        # Deleting a secret with consumers
        if force:
            self.manager.delete(secret_ref=secret_ref, force=True)
        else:
            self.assertRaises(ValueError, self.manager.delete,
                              secret_ref=secret_ref)

    def test_should_delete_from_manager_consumers_no_force(self):
        self._test_should_delete_from_manager_with_consumers()

    def test_should_delete_from_manager_consumers_force(self):
        self._test_should_delete_from_manager_with_consumers(self.entity_id,
                                                             True)

    def test_should_delete_from_manager_using_stripped_uuid(self):
        bad_href = "http://badsite.com/" + self.entity_id
        self.test_should_delete_from_manager(secret_ref=bad_href)

    def test_should_delete_from_manager_using_only_uuid(self):
        self.test_should_delete_from_manager(secret_ref=self.entity_id)
