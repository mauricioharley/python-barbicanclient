...
from barbicanclient.client import _DEFAULT_API_MICROVERSION
...
class Client(object):
    ...
    def __init__(self, session=None, *args, **kwargs):
        ...
        """
        microversion = kwargs.pop('microversion', _DEFAULT_API_MICROVERSION)
        # In case microversion was set to None kwargs.pop will return None
        if microversion is None:
            microversion = _DEFAULT_API_MICROVERSION

        if not self._is_supported_microversion(
            session,
            kwargs.get('endpoint'),
            kwargs.get('version'),
        ...
        self.client = base_client._HTTPClient(session, microversion,
                                              *args, **kwargs)
    ...
    def _is_supported_microversion(self, session, endpoint, version,
                                   service_type, service_name, interface,
                                   region_name, microversion):
        # first we make sure that the microversion is something this client
        # understands
